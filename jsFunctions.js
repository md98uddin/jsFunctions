...
//sorts an object by its population attribute
...
//checks if a nth degree number equals to sum of its individual digit power to nth
...
//checks if a brace string eg "(({{}}))" has even amount of braces
function matchBrackets(str){
    ...
    for(let i=0; i<braces.length ;i=i+2){
        if(map.get(braces[i])!==map.get(braces[i+1]))
            valid=false;
    }
...
//check if a string is read same backward and forward
